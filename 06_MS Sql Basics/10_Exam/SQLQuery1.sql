...
WHERE 
	DepartmentId = 
	(SELECT Id FROM VolunteersDepartments WHERE DepartmentName = 'Education program assistant')
...
SELECT a.[Name], at.AnimalType, FORMAT(a.BirthDate, 'dd.MM.yyyy') AS BirthDate FROM Animals a
JOIN AnimalTypes [at] ON a.AnimalTypeId = at.Id
ORDER BY a.[Name]

--07. Owners and Their Animals

SELECT TOP (5) o.[Name] AS [Owner], COUNT(*) AS CountOfAnimals FROM Owners o
...
SELECT CONCAT(o.[Name],'-', a.[Name]) AS OwnersAnimals, o.PhoneNumber, c.Id AS CageId FROM Owners AS o
...
SELECT v.[Name], v.PhoneNumber, REPLACE((REPLACE(v.[Address], ',', '')), 'Sofia', '') [Address] FROM Volunteers v
...
SELECT a.[Name], YEAR(a.BirthDate) BirthYear, [at].AnimalType FROM Animals a
JOIN AnimalTypes [at] ON a.AnimalTypeId = [at].Id
WHERE a.OwnerId IS NULL AND [at].AnimalType != 'Birds' AND DATEDIFF(YEAR, a.BirthDate, '2022/01/01') < 5
ORDER BY a.[Name]


--11. All Volunteers in a Department

CREATE FUNCTION udf_GetVolunteersCountFromADepartment (@departmentName VARCHAR(30))
RETURNS INT AS
BEGIN 

	DECLARE @Count INT = 
	(SELECT COUNT(*) FROM Volunteers WHERE DepartmentId = 
	(SELECT Id FROM VolunteersDepartments WHERE DepartmentName = @departmentName))
...
ELSE
BEGIN
SET @OwnerName = (SELECT [Name] FROM Owners WHERE Id = @OwnerId)
END

SELECT @AnimalName [Name], @OwnerName [OwnersName]
